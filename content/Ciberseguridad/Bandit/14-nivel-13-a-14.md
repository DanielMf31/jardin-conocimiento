...
> ⚠️ **Cambio reciente de OverTheWire**: el método clásico (`ssh ... bandit14@localhost`) **ya no funciona** porque ahora **bloquean conectarse a `localhost:2220` desde dentro del servidor**. Hay que hacerlo **desde tu propia máquina** (ver Paso 3).
...
> ⚠️ El método clásico `ssh -i sshkey.private bandit14@localhost -p 2220` desde dentro del servidor **ahora está BLOQUEADO**: verás `Connecting from localhost is blocked to conserve resources`. Hay que hacerlo **desde tu propia máquina**.
