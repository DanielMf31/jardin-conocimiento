...
> ⚠️ **Bloqueo de `localhost`** (igual que en [[14-nivel-13-a-14]]): OverTheWire **bloquea `ssh ...@localhost:2220` desde dentro del servidor** (`Connecting from localhost is blocked to conserve resources`). Usa el **hostname real** `bandit.labs.overthewire.org`:
